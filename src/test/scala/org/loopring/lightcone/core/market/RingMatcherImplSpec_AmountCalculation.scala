...
class RingMatcherImplSpec_AmountCalculation extends CommonSpec {
  ...
  var decimal = "000000000000000000000000000000"
  ...
  "price3 just match" should "part fill if both matchables are a third of raw amount" in {
    ...
    val res = matcher.matchOrders(
      taker.copy(_matchable = Some(OrderState(amountS = BigInt("333333333333333333333333333"), amountB = BigInt("3333333333333333333333333330"), amountFee = BigInt("333333333333333333")))),
      maker.copy(_matchable = Some(OrderState(amountS = BigInt("3333333333333333333333333333"), amountB = BigInt("333333333333333333333333333"), amountFee = BigInt("333333333333333333"))))
    )
    val expectRing = OrderRing(
      taker = ExpectedFill(
        order = taker.copy(_matchable = Some(OrderState())),
        pending = OrderState(amountS = BigInt("333333333333333333333333333"), amountB = BigInt("3333333333333333333333333330"), amountFee = BigInt("333333333333333333")),
        amountMargin = BigInt(0)
      ),
      maker = ExpectedFill(
        order = maker.copy(_matchable = Some(OrderState(amountS = 3, amountFee = 1))),
        pending = OrderState(amountS = BigInt("3333333333333333333333333330"), amountB = BigInt("333333333333333333333333333"), amountFee = BigInt("333333333333333332")),
        amountMargin = BigInt(0)
      )
    )
    res.right.toOption should be(Some(expectRing))
  ...
  "price2 just match" should "fill with different decimal and raw amount isn't divisible by matchable " in {
    ...
    val taker = buyDAI(takerAmountS, takerAmountB, takerFee)
    val res = matcher.matchOrders(
      taker.copy(_matchable = Some(OrderState(amountS = BigInt("333333333"), amountB = BigInt("33333333300000000000"), amountFee = BigInt("3030303027272727272")))),
      maker.copy(_matchable = Some(OrderState(amountS = BigInt("33333333333333333333"), amountB = BigInt("333333333"), amountFee = BigInt("3030303030303030303"))))
    )
    val expectRing = OrderRing(
      taker = ExpectedFill(
        order = taker.copy(_matchable = Some(OrderState())),
        pending = OrderState(amountS = BigInt("333333333"), amountB = BigInt("33333333300000000000"), amountFee = BigInt("3030303027272727272")),
        amountMargin = BigInt(0)
      ),
      maker = ExpectedFill(
        order = maker.copy(_matchable = Some(OrderState(amountS = BigInt("33333333333"), amountFee = BigInt("3030303031")))),
        pending = OrderState(amountS = BigInt("33333333300000000000"), amountB = BigInt("333333333"), amountFee = BigInt("3030303027272727272")),
        amountMargin = BigInt(0)
      )
    )
    res.right.toOption should be(Some(expectRing))
  ...
  "price1 with margin" should " fill if matchable is raw amount" in {
    ...
    val expectRing = OrderRing(
      ...
      maker = ExpectedFill(
        order = maker.copy(_matchable = Some(OrderState(amountS = BigInt("700000000000" + decimal), amountB = BigInt("42000000000" + decimal), amountFee = BigInt("42" + decimal)))),
        pending = OrderState(amountS = BigInt("50000000000" + decimal), amountB = BigInt("3000000000" + decimal), amountFee = BigInt("3" + decimal)),
        amountMargin = BigInt(0)
      )
    )
    res.right.toOption should be(Some(expectRing))
