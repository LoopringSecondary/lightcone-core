...
class PendingRingPoolSpec extends FlatSpec with Matchers {
  ...
  "testBaseOperation" should "add ring" in {
    ...
    (0 until 10) foreach {
      i ⇒
        val makerExpectFill = ExpectedFill(
          order = Order(id = "maker-" + i, tokenS = lrc, tokenB = eth, tokenFee = lrc, walletSplitPercentage = 0.2),
          pending = OrderState(amountS = 200, amountFee = 200),
          amountMargin = 100
        )
        val takerExpectFill = ExpectedFill(
          order = Order(id = "taker-" + i, tokenS = eth, tokenB = lrc, tokenFee = lrc, walletSplitPercentage = 0.2),
    ...
    val makerExpectFill = ExpectedFill(
      order = Order(id = "maker-1", tokenS = lrc, tokenB = eth, tokenFee = lrc, walletSplitPercentage = 0.2),
      pending = OrderState(amountS = 200, amountFee = 200),
      amountMargin = 100
    )
    val takerExpectFill = ExpectedFill(
      order = Order(id = "taker-1", tokenS = eth, tokenB = lrc, tokenFee = lrc, walletSplitPercentage = 0.2),
      pending = OrderState(amountS = 100, amountFee = 100),
      amountMargin = 100
    )
    val ring = OrderRing(
