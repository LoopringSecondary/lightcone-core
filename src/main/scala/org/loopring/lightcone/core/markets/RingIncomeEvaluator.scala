...
final class RingIncomeEstimatorImpl(
  ...
  def getIncomeFiatValue(ring: OrderRing) = ring.maker.getIncomeFiatValue() + ring.taker.getIncomeFiatValue()

  def isProfitable(ring: OrderRing) = getIncomeFiatValue(ring) >= threshold
}
